										...
										<div class="container-fluid">
									    	...
									    	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
									    		<ul class="nav navbar-nav">
									        		...
									        		<li><a href="{{ url('/admin/print/indexNf') }}"><i class="material-icons">money_off</i>No facturado</a></li>
									    		</ul>
									    	</div>
										</div>
									</nav>
				                    <form class="form-inline" method="POST" action="{{ url('/admin/print/indexNf') }}">
                        				@csrf
						                ...
						                <table class="table table-bordered table-dark">
										    ...
										    <tbody>
										        ...
										        <tr>
										            ...
										            <td class="info-title" data-currency="MXN">{{$billedExpense->driver_food}}</td>
										            <td class="info-title" data-currency="MXN">{{$billedExpense->carrier1_salary}}</td>
										            <td class="info-title" data-currency="MXN">{{$billedExpense->carrier1_food}}</td>
										            <td class="info-title" data-currency="MXN">{{$billedExpense->carrier2_salary}}</td>
										            <td class="info-title" data-currency="MXN">{{$billedExpense->carrier2_food}}</td>
										            ...
										            <td class="info-title" data-currency="MXN">{{$billedExpense->maneuvers}}</td>
													<td class="info-title" data-currency="MXN">{{$billedExpense->passages}}</td>
													...
													<td class="info-title" data-currency="MXN">{{$billedExpense->parking}}</td>
													<td class="info-title" data-currency="MXN">{{$billedExpense->spare_parts}}</td>
													<td class="info-title" data-currency="MXN">{{$billedExpense->mulcts}}</td>
													<td class="info-title" data-currency="MXN">{{$billedExpense->cargo_van}}</td>
													<td class="info-title" data-currency="MXN">{{$billedExpense->boardinghouse}}</td>
										            ...
										            <td class="info-title" class="td-actions text-right">
										                <a type="button" rel="tooltip" title="Imprimir" class="btn btn-info btn-simple btn-xs" data-placement="right" href="{{ url('/admin/print/'.$billedExpense->id.'/vsearch1') }}">
